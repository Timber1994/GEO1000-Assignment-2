...
def move(start, end, moves):
    ...
    Arguments:
        start: The starting point of the robot, later it takes the current position.
        end: The end point of the robot.
        moves: The number of moves required (or left), these can be right (+) moves or left (-) moves.
    Returns:
    ...
    if abs(end-start) > moves:                      # If the distance is still larger than the no. of moves left it fails
        return 0
